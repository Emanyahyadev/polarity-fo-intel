...
Six real failures, all found by running the thing rather than reasoning about it:

1. **`principal_email` held generic firm inboxes.** A firm's `info@` was written straight into a field
   named for a person. Found by audit, fixed in `ed077b6` (split into `firm_contact_email`).
2. **NVIDIA free-tier latency (130-380s per call)** — discovered empirically, and it directly shaped the
   design: two LLM calls per *goal run*, never per candidate, both grounded and rejectable.
3. **Sector-match scoring penalised mandates that named no sector**, capping every record at "thin" even
   with 2+ sources. Found by reading Goal 1's actual output (`goal-923b73bc7c.json`, kept as the
   pre-fix artifact) rather than trusting that it "looked fine". Fixed in `2bfdc0a`.
4. **`FreshnessAgent` hardcoded `stale: []`** — the cross-run staleness requirement had *no*
   implementation at all behind a step that reported success. Found by reading the code, not the status.
5. **Two workflow bugs silently breaking every scheduled run's commit**: an unmatched `data/freshness/*`
   glob aborted the whole `git add` atomically (taking `data/final/*` with it), and `notes/*.md`
   regenerated by `generate_history.py` was never staged, leaving the tree dirty so `git pull --rebase`
   failed and the push was rejected. Found by watching runs `31395597811` and `31396664044` fail and
   reading `gh run view --log-failed`. Fixed in `c9b42db` and `8e5c5a1`.
6. **The deployed image was missing `data/final/records.json`**, so `POST /goal` 500'd on the live
   service while working locally — the Dockerfile only ever copied the CSV, which `/query` needs but the
   agent's full-fidelity store does not. Caught by curling the live endpoint immediately after deploy
   instead of assuming a green build meant a working feature. Fixed in `bec44f4`.

Two fabricated UI panels ("AI Employee Status" with hardcoded agent states, "AI Operating Cycle" as a
decorative pulse) were also found and deleted (`07cd91c`) — they are the exact failure mode the brief
names, and no amount of relabelling would have made them real.

## 7 · Buyer challenge and demonstrated commercial value

**Customer:** an IR lead or GP raising a fund, working a prospect list of family offices.
**Tier:** this is the paid tier's justification — the free/Stage-1 tier answers "which firms match X";
this answers "what should I do next, and why should I believe you".
**Part of the root challenge served:** *prioritise* and *reach* — specifically the judgement burden of
deciding where to spend limited research hours.

The burden removed is concrete and measurable in the artifacts. Goal 3
(`reports/goals/goal-99f7644650-GOAL3-LIVE.json`) answers a question the Stage 1 RAG structurally
cannot: *which firms are worth my team's manual research time?* It cross-references evidence strength
against contactability and returns a ranked worklist — e.g. WE Family Offices ranks first because it
carries two independent sources and a stated thesis **and** a named decision-maker (Santiago Ulloa) with
no contact route on file, making it the highest-yield target for a human researcher. Doing this by hand
means opening 80 records, judging each one's evidence, checking each for a reachable person, and ranking
the gap — the agent does it in ~6 seconds with the reasoning shown per firm.

**Where the value is visible in the live product:** the Agent tab states the customer's task in plain
language ("Give the agent a goal, not a search"), shows *what the agent did* (records considered, how
many had strong vs thin evidence), renders each result with its evidence count, its uncertainty in words
not jargon ("Thin evidence — treat as a lead, not a conclusion"), and a next step. The Directory tab's
"Who can you actually contact?" panel makes the dataset's biggest limitation the first thing a buyer
sees, rather than burying it.

**What remains unsolved, plainly:** the dataset is 80 records, not 500, and **zero** of them currently
carry a verified route to a named individual — so today the agent is very good at telling a customer
*where the gaps are* and much weaker at closing them. The contact-enrichment path exists in code
(`enrichment/person_contact.py`) but has not yet produced a real named-person route against live sites.
**Would I charge for this today? Not at full price.** It is worth paying for as a research-triage tool
for a team that already has its own contact-sourcing motion; it is not yet worth paying for as a
contact database, because on that dimension it currently delivers nothing.
