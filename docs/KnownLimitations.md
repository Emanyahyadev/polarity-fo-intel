...
- **Principal / AUM / recent-investment depth is 13F-bounded (~26/50).** Records that file SEC Form 13F carry authoritative, dated **principal (name/title/direct phone)**, **AUM (aggregate 13(f) securities value)**, and **recent investments (new positions vs the prior quarter)** — pulled from the filing itself. Two honest caveats travel with each: the principal is the **13F signatory** (a named officer, title as filed — often the CCO/GC, *not necessarily the lead investor*), and the AUM is **13(f) securities value, not total AUM**. Firms that do not file 13F (~24/50) keep principal + AUM as honest `could_not_verify` rather than a scraped or guessed value.
- **AUM is 13F-only, by choice.** Website-stated AUM was **removed** — scraped marketing figures proved unreliable (industry totals / parent-firm numbers, e.g. a bogus "$35B"). Stale 13F filings (>~25 months) are also dropped, so no out-of-date value is presented as current. The result is fewer AUMs but every one authoritative and dated.
- **Investment thesis is ~17/50.** An attributable sentence is extracted from firms with a clean website "about/approach" statement; navigation-menu text is rejected, so a firm with no clean statement honestly gets none.
