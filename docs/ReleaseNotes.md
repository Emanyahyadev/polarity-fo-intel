...
| 7 | Stale doc counts | Reconciled every doc to **55 records / 23-23 RAG eval / recall 0.50 / 8 FNs**. | docs sweep |
...
- **RAG grounding/abstention eval:** **23/23** — includes international queries and a new regression test that a reclassification left **no true SFO in Texas** (so `single-family offices in Texas` correctly abstains).
...
- [x] Documentation consistent (55 / 23-23 / recall 0.50 everywhere current)
