...
- **Health:** `GET /health` → `{"status":"ok","records":60}`
...
- `data/final/family_offices.csv` — the 60 validated records the RAG answers from.
...
# GET  http://localhost:8000/health      -> {"status":"ok","records":60}
