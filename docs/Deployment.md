...
- **Health:** `GET /health` → `{"status":"ok","records":57}`
...
- `data/final/family_offices.csv` — the 57 validated records the RAG answers from.
...
# GET  http://localhost:8000/health      -> {"status":"ok","records":57}
