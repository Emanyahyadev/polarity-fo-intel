...
- **Health:** `GET /health` → `{"status":"ok","records":55}`
...
- `data/final/family_offices.csv` — the 55 validated records the RAG answers from.
...
# GET  http://localhost:8000/health      -> {"status":"ok","records":55}
