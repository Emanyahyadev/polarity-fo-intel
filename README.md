...
A production-shaped pipeline that discovers, enriches, and **validates** a decision-grade dataset of 57 family offices, then serves it through a grounded Micro-RAG with a non-technical, customer-facing UI.
...
| Deliverable | Where |
|---|---|
| Dataset — 57 validated records (8 High / 35 Medium / 14 Low confidence) | `data/final/family_offices.xlsx` / `.csv` |
...
| Discovery report (398 → 57, with rejections) | `docs/evidence/dataset-discovery-report.json` |
...
- **Rule 2 (firms):** a firm counts toward the 57 only with affirmative evidence it *is* a family office; SFO/MFO/Undetermined is labelled honestly, never relabelled.
- **Findings govern releases:** anything that fails validation is withheld from delivered fields and recorded in an audit trail.
- **Multi-source & independent:** the delivered set is discovered via SEC EDGAR (13F, 28), SEC IAPD / Form ADV registration (20), and a curated Wikipedia/Wikidata reference lens (9); every fact is *verified* against an authoritative source of a **different class than discovery** (47/57), never one source copied at scale. (SEC dominates free-tier US discovery — an honest, disclosed skew.)
